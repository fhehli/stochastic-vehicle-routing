...
def plot_city_edges(city: City, E: list[Edge], ax: Axes, title: str, tasks_only: bool):
    ...
    handles = [Line2D([], [], color=color, label=label) for color, label in zip(colors, labels)]
...
def visualize_tasks(
    city: City, vsp_path: list = None, show_heatmap: bool = False, show_reachable_paths: bool = False
) -> Figure:
    """
    ...
    fig = plt.figure(figsize=(20, 15))
    ax = fig.add_subplot(111)
    colors = [cm(1.0 * i / (n_tasks)) for i in range(n_tasks)]
...
def visualize_tasks_nx():
    ...
    nx.draw(G=G, pos=pos, with_labels=True, node_size=400, node_color="skyblue", font_size=10, arrows=True)
...
def visualize_paths_nx(G: SimpleDirectedGraph, vsp_path: list[int]):
    ...
    node_path = set()
    edge_path = [(E[i].from_vertex.name, E[i].to_vertex.name) for i, v in enumerate(vsp_path) if v == 1]
    for e in edge_path:
        node_path.add(e[0])
        node_path.add(e[1])
    ...
    nx.draw_networkx_edges(
        nxgraph, pos, edgelist=set(nxgraph.edges) - set(edge_path), connectionstyle="arc3, rad = 0.3"
    )
    ...
    nx.draw(G=nxgraph, pos=pos, with_labels=True, node_size=400, node_color="skyblue", font_size=10, arrows=True)
